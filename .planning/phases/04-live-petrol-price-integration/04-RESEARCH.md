...
**Confidence:** MEDIUM — Original recommendation was fuelprice.io ($49.50/yr — rejected). **API DECISION UPDATED 2026-04-06:** Using Servo Saver Public API (Victorian Government, free). Consumer ID applied for and pending approval. Endpoint/response schema will be confirmed from API docs once approved.
...
|----|-------------|------------------|
| FUEL-API | Fetch live average Australian fuel prices from a national API keyed to the trip creator's homebase suburb/postcode | **Servo Saver Public API** (Victorian Government, free) — Consumer ID applied for, pending approval. fuelprice.io was original recommendation but costs $49.50/yr (rejected). |
